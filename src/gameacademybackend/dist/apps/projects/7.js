...
var ProviderError = __webpack_require__(2577);
...
function httpRequest(options) {
    return new Promise((resolve, reject) => {
        ...
        req.on("error", (err) => {
            reject(Object.assign(new ProviderError.ProviderError("Unable to connect to instance metadata service"), err));
            req.destroy();
        });
        req.on("timeout", () => {
            reject(new ProviderError.ProviderError("TimeoutError from instance metadata service"));
            req.destroy();
        });
        req.on("response", (res) => {
            const { statusCode = 400 } = res;
            if (statusCode < 200 || 300 <= statusCode) {
                reject(Object.assign(new ProviderError.ProviderError("Error response received from instance metadata service"), { statusCode }));
                req.destroy();
            }
            const chunks = [];
            res.on("data", (chunk) => {
...
const fromContainerMetadata = (init = {}) => {
    ...
    return () => retry(async () => {
        ...
        if (!isImdsCredentials(credsResponse)) {
            throw new CredentialsProviderError.CredentialsProviderError("Invalid response received from instance metadata service.", {
                logger: init.logger,
            });
        }
...
const getCmdsUri = async ({ logger }) => {
    ...
    if (process.env[ENV_CMDS_FULL_URI]) {
        ...
        if (!parsed.hostname || !(parsed.hostname in GREENGRASS_HOSTS)) {
            throw new CredentialsProviderError.CredentialsProviderError(`${parsed.hostname} is not a valid container metadata service hostname`, {
                tryNextLink: false,
                logger,
            });
        }
        if (!parsed.protocol || !(parsed.protocol in GREENGRASS_PROTOCOLS)) {
            throw new CredentialsProviderError.CredentialsProviderError(`${parsed.protocol} is not a valid container metadata service protocol`, {
                tryNextLink: false,
                logger,
            });
        }
    ...
    throw new CredentialsProviderError.CredentialsProviderError("The container metadata credential provider cannot be used unless" +
...
var dist_es = __webpack_require__(2566);
...
const getInstanceMetadataEndpoint = async () => (0,dist_es.parseUrl)((await getFromEndpointConfig()) || (await getFromEndpointModeConfig()));
const getFromEndpointConfig = async () => (0,configLoader.loadConfig)(ENDPOINT_CONFIG_OPTIONS)();
const getFromEndpointModeConfig = async () => {
    const endpointMode = await (0,configLoader.loadConfig)(ENDPOINT_MODE_CONFIG_OPTIONS)();
    switch (endpointMode) {
...
const getInstanceMetadataProvider = (init = {}) => {
    ...
    const getCredentials = async (maxRetries, options) => {
        ...
        if (isImdsV1Fallback) {
            ...
            const configValue = await (0,configLoader.loadConfig)({
                environmentVariableSelector: (env) => {
                    const envValue = env[AWS_EC2_METADATA_V1_DISABLED];
                    fallbackBlockedFromProcessEnv = !!envValue && envValue !== "false";
                    if (envValue === undefined) {
                        throw new CredentialsProviderError.CredentialsProviderError(`${AWS_EC2_METADATA_V1_DISABLED} not set in env, checking config file next.`, { logger: init.logger });
                    }
