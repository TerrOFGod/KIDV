...
|||||
|-|-|-|-|
|Windows| .Net Core | > ```dotnet build Account.sln``` |
|| Docker | > ```docker build -t User .``` |
...
|||||
|-|-|-|-|
|Windows| .Net Core | > ```dotnet run --project src\Account.Api\``` |
|| Docker | > ```docker run -d -e [envar=value] -p 5000:80 User```|
